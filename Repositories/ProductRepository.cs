...
namespace Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            var result = await _clothesStoreDbContext.Products.ToListAsync(cancellationToken);
            return (1, result);
        }
        public async Task<(int, List<Product>)> GetProductByIdAsync(int id, CancellationToken cancellationToken)
        {
            var result = await _clothesStoreDbContext.Products.Where(p => p.ProductId == id).ToListAsync(cancellationToken);
            return (1, result);
        }
        public async Task<(int, Product)> InsertProductAsync(Product product, CancellationToken cancellationToken)
        {
            await _clothesStoreDbContext.Products.AddAsync(product, cancellationToken);
        ...
        {
            await _clothesStoreDbContext.Products.AddRangeAsync(products, cancellationToken);
            _clothesStoreDbContext.SaveChanges();
            return (1, products);
        }
